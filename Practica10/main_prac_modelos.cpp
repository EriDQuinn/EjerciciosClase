...
//For model
float movKit_z = 0.0f;
float rotrueda = 0.0f,dir=0.0f, dirRueda = 0.0f;
...
void animate(void)
{
	//estructura para controlar la secuencia de animacion
	if (play) {
			movKit_z += dir;
			rotrueda += dirRueda;
	}
...
void display(Shader shader, Model modelo, Model llantas)
{
	...
	model = glm::translate(tmp, glm::vec3(3.7f, -0.5f, 2.5f));
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	model = glm::rotate(model, glm::radians(rotrueda), glm::vec3(1.0, 0.0, 0.0));
	shader.setMat4("model", model);
	llantas.Draw(shader);	//Izq delantera

	model = glm::translate(tmp, glm::vec3(-1.67f, -0.5f, 2.5f));
	model = glm::rotate(model, glm::radians(rotrueda), glm::vec3(1.0, 0.0, 0.0));
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	shader.setMat4("model", model);
	llantas.Draw(shader);	//Der delantera

	model = glm::translate(tmp, glm::vec3(-1.67f, -0.5f, -6.2f));
	model = glm::rotate(model, glm::radians(rotrueda), glm::vec3(1.0, 0.0, 0.0));
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	shader.setMat4("model", model);
	llantas.Draw(shader);	//Der trasera

	model = glm::translate(tmp, glm::vec3(3.7f, -0.5f, -6.2f));
	model = glm::rotate(model, glm::radians(rotrueda), glm::vec3(1.0, 0.0, 0.0));
	model = glm::scale(model, glm::vec3(0.02f, 0.02f, 0.02f));
	shader.setMat4("model", model);
